...
const port2 = config_1.PORT2;
const httpServer = new http_1.Server(app_1.default);
const server = https_1.createServer("", app_1.default);
...
server.listen(config_1.PORT2, (err) => {
    if (err) {
        return logger_1.logger.error(err);
    }
    return logger_1.logger.info(`https Server is listening on port: ${port2}`);
});
